...
//			System.out.println("GOAL STATE REACHED! NV: " + 0);
		...
		if (gn <= 6000 || (!goalReached)) {
			...
			Instant endTime = Instant.now();
			Duration searchTime = Duration.between(startTime, endTime);
			runInformation.add(Integer.toString(gn));
			runInformation.add(Integer.toString(curr.getCost()));
			runInformation.add(Integer.toString(searchTime.getNano()));
		}
